...
namespace Game.View
{
    ...
    /// 세로 막대: 가드 게이지(하양, 4등분 눈금) + 옆에 질풍참 스택/쿨타임(빨강).
    ...
    public class CombatHud : MonoBehaviour
    {
        ...
        void DrawGuard(float x, float y, in PlayerCombatState c)
        {
            var bg = new Color(0.08f, 0.08f, 0.10f, 0.85f);
            Fill(x - 2, y - 2, BarW + 4, BarH + 4, bg);

            float f = Mathf.Clamp01(c.guardGauge / (float)SimConfig.GuardMax);
            float fh = BarH * f;
            var white = c.blocking ? new Color(0.7f, 0.9f, 1f) : new Color(0.92f, 0.92f, 0.95f);
            Fill(x, y + (BarH - fh), BarW, fh, white);   // 아래에서 위로 차오름

            // 4등분 눈금 (칼등치기 1회 = 1/4)
            var mark = new Color(0.15f, 0.15f, 0.18f, 0.9f);
            for (int i = 1; i < 4; i++)
                Fill(x, y + BarH * (i / 4f) - 1f, BarW, 2f, mark);
