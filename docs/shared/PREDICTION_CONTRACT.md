...
- 화면 중앙에는 바깥 링이 판정 링으로 수축하는 타이밍 UI를 표시한다. 중앙은 현재 입력,
  상단 레일은 현재부터 최대 3개의 입력 순서를 표시하며 동일 위치 연속 입력은
  `RAPID COMBO`로 명시한다. 월드 잔상은 위치, 중앙 링은 시점, 레일은 순서를 담당한다.
...
- 액션 대기 판정은 실제 시간 0.42초를 Good 한계로 사용한다. Miss면 해당 명령을 실행하지
  않고 현재 `SimWorld`에서 직접 조작으로 복귀한다. 실행 최저 배속은 `0.35`이며,
